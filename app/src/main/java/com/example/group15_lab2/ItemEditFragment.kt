...
class ItemEditFragment : Fragment() {
    ...
    private fun populateTextView(item: Item) {
        ...
        if (item.delivery != null) {
            val deliveryPosition =
                resources.getStringArray(R.array.Currencies).indexOf(item.delivery)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.save_button -> {
                val snack:Snackbar = Snackbar.make(item_title_edit, "", Snackbar.LENGTH_LONG)
                ...
                if (item_title_edit.text.toString().length > 15) {
                    ...
                    snack.setText("Too many characters in the Title")
                    tv.setTextColor(Color.WHITE)
                    tv.typeface = Typeface.DEFAULT_BOLD
                    snack.view.setBackgroundColor(ContextCompat.getColor(context!!, R.color.longTitle))

                } else {
                    savePreferences(snack,tv)
    ...
    private fun savePreferences(snack: Snackbar, tv: TextView) {
        ...
        if (new_item) {
            ...
            tv.setTextColor(Color.WHITE)
            tv.typeface = Typeface.DEFAULT_BOLD
            snack.view.setBackgroundColor(ContextCompat.getColor(context!!, R.color.newItem))
        } else {
